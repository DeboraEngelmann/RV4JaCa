...
manage_event(WebSocket) :-
    ...
    (Msg.opcode==close ->
         ...
         (next(TE1, E, TE2) ->
           ...
           (nb_getval(init_state,TEinit), nb_setval(state, TEinit), writeln('verdict = False'), term_string(TE1, TE1Str), Reply=(_{}.put(E).put(_{verdict:false, spec:TE1Str})))), % Verdict = False
         atom_json_dict(Json,Reply,[as(string)]),
	       ws_send(WebSocket, string(Json)),
	       manage_event(WebSocket)).


exception(undefined_global_variable, state, retry) :- trace_expression(_, TE), nb_setval(state,TE), nb_setval(init_state,TE).
